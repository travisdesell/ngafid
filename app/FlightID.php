...
class FlightID extends Model {
	 ...
	 * @var array
	 */
	protected $fillable = ['n_number', 'time', 'date', 'fleet_id', 'aircraft_type', 'origin', 'destination'];
    ...
    public function scopeFlightDetails($query, $fleet, $startDate = null, $endDate = null, $archived = '', $sort = null, $column = null, $duration = '00:00')
    {
        /*return $query->select('flight_id.id', 'n_number', 'date', 'time', 'origin', 'destination', 'duration',
            ...
            ->distinct()
            ->leftJoin('main_exceedances','main_exceedances.flight','=','flight_id.id')
            ->leftJoin('aircraft_list','aircraft_list.id','=','flight_id.aircraft_type')
            ->where('flight_id.fleet_id', '=', $fleet)
            ->orderBy('flight_id.date', 'desc')
            ->orderBy('flight_id.time', 'desc');*/

        $query->select('flight_id.id', 'n_number', 'date', 'time', 'origin', 'destination', 'duration',
            'aircraft name', 'year', 'make', 'model', \DB::raw("COALESCE(num_exceedances, 0) num_events"),
            \DB::raw("main_exceedances.archived AS 'archived'"))
              ->distinct();
        $query->leftJoin('main_exceedances','main_exceedances.flight','=','flight_id.id');
        $query->leftJoin('aircraft_list','aircraft_list.id','=','flight_id.aircraft_type');
        $query->where('flight_id.fleet_id', '=', $fleet);


        if($archived != '')
        {
            $query->where(\DB::raw('main_exceedances.`archived`'), '=', \DB::raw("'" . $archived . "'") );
        }

        if($startDate != ''){
            $query->where(\DB::raw('flight_id.`date`'), '>=', \DB::raw("'" . $startDate . "'") );
        }

        if($endDate != ''){
            $query->where(\DB::raw('flight_id.`date`'), '<=', \DB::raw("'" . $endDate . "'") );
        }

        if($column != '')
        {
            $query->where(\DB::raw('main_exceedances.' . $column), '=', \DB::raw("'Y'"));
        }
        ...
        switch($sort)
        {
            case 1: $query->orderBy('num_exceedances', 'DESC');
                break;

            case 2: $query->orderBy('date', 'ASC');
                    $query->orderBy('time', 'ASC');
                break;

            case 3: $query->orderBy('destination', 'ASC');
                break;

            case 4: $query->orderBy('origin', 'ASC');
                break;

            case 5: $query->orderBy('duration', 'ASC');
                break;
...
//dd($query->toSql());
    ...
    /*public function scopeAircraftExceedance($query, $fleet, $exceedanceColumn)
    {
        return $query->select('flight_id.id', 'n_number', 'date', 'time', 'origin', 'destination', 'duration',
            'aircraft name', 'year', 'make', 'model', \DB::raw("COALESCE(num_exceedances, 0) num_events"))
            ->distinct()
            ->join('main_exceedances','main_exceedances.flight','=','flight_id.id')
            ->leftJoin('aircraft_list','aircraft_list.id','=','flight_id.aircraft_type')
            ->where('flight_id.fleet_id', '=', $fleet)
            ->where(\DB::raw('main_exceedances.' . $exceedanceColumn), '=', \DB::raw("'Y'"))
            ->where(\DB::raw('main_exceedances.archived'), '=', \DB::raw("'N'"))
            ->orderBy('flight_id.date', 'desc')
            ->orderBy('flight_id.time', 'desc');
    }*/
    ...
    {
        return $query->select(\DB::raw("SUM(TIME_TO_SEC(duration)) AS 'duration'"))
                    ->where('fleet_id', '=', "{$fleet}");
    }
